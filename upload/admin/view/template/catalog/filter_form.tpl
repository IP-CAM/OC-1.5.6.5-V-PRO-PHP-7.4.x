    ...
    <div class="content">
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        ...
        <table id="filter" class="list">
          <thead>
            <tr>
              <td style="text-align:left;"><span class="required">*</span> <?php echo $entry_name ?></td>
              <td style="text-align:right;"><?php echo $entry_sort_order; ?></td>
              <td></td>
            </tr>
          </thead>
          <?php $filter_row = 0; ?>
          ...
          <tbody id="filter-row<?php echo $filter_row; ?>">
            <tr>
              <td style="text-align:left;"><input type="hidden" name="filter[<?php echo $filter_row; ?>][filter_id]" value="<?php echo $filter['filter_id']; ?>" />
                ...
                <?php } ?>
                <?php } ?></td>
              <td style="text-align:right;"><input type="text" name="filter[<?php echo $filter_row; ?>][sort_order]" value="<?php echo $filter['sort_order']; ?>" size="1" /></td>
              <td style="text-align: center;"><a class="button" onclick="$('#filter-row<?php echo $filter_row; ?>').remove();" class="button"><?php echo $button_remove; ?></a></td>
            </tr>
          </tbody>
          <?php $filter_row++; ?>
          <?php } ?>
          <tfoot>
            <tr>
              <td colspan="2"></td>
              <td style="text-align:left;"><a onclick="addFilter();" class="button"><?php echo $button_add_filter; ?></a></td>
            </tr>
          </tfoot>
        </table>
      </form>
...
function addFilter() {
	...
	html += '  <tr>';	
    html += '    <td style="text-align:left;"><input type="hidden" name="filter[' + filter_row + '][filter_id]" value="" />';
	<?php foreach ($languages as $language) { ?>
    ...
    <?php } ?>
	html += '    </td>';
	html += '    <td style="text-align:right;"><input type="text" name="filter[' + filter_row + '][sort_order]" value="" size="1" /></td>';
	html += '     <td style="text-align:left;"><a onclick="$(\'#filter-row' + filter_row + '\').remove();" class="button"><?php echo $button_remove; ?></a></td>';
	html += '  </tr>';	
    html += '</tbody>';
