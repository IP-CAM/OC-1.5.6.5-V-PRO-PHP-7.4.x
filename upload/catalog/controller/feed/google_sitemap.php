		...
		if ($this->config->get('google_sitemap_status')) {
			...
			foreach ($products as $product) {
				if ($product['image']) {
					...
					$output .= '  <image:image>';
					$output .= '  <image:loc>' . htmlspecialchars($this->model_tool_image->resize(html_entity_decode($product['image'], ENT_QUOTES, 'UTF-8'), $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height')), ENT_COMPAT | ENT_XML1) . '</image:loc>';
